...
Advance along [[Prism (viewpoint layer)|prism.en]] → [[Loom (transitional structure layer)|loom.en]] → [[HTN/HDDL (full structure layer)|planner.en]], letting the planner handle as much of workflow creation, improvement, and maintenance as possible. Continuous improvement based on OpenTelemetry execution history also lands in this stage.

## The Ultimate Goal

**Fully automatic improvement, no human in the loop.** Further out lies the cognitive-model / world-constraint layer (see [[Prism Reasoning|prism.en]], Section 9) — a planner that knows "what is objectively impossible." Currently open research.

## The Community Feedback Most Wanted

- Verify or falsify Prism's "flattening" phenomenon with **complex, real-world tasks** ([[Prism Reasoning|prism.en]], Sections 3 and 6);
...
Behind this are two routes for approximating an answer: a neural network **fixes the coordinate system** and fits a composite function by gradient descent; logical reasoning **fixes the function and transforms the coordinate system** — repeatedly changing dimensions (changing basis) until the problem becomes solvable under the right coordinates. The two converge on the same destination (I conjecture their expressive power is equivalent — an open hypothesis, see [[Prism Reasoning|prism.en]], Section 4), but their processes and principles are entirely different. Unigen attempts to connect the two routes: neurons handle generation *within* dimensions; logic handles the selection, transformation, and composition *of* dimensions — call it **logic-oriented AI**.
