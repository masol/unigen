 ...
 *
 * 扩展点：按 node.kind 追加撰写偏好与质量兜底，见 kindGuidance.ts。
 */
...
/**
 * qualityCriteria 缺失时的降级顺序：
 *   1. artifact 自带 qualityCriteria
 *   2. 下游 intent 反推
 *   3. 按 node.kind 的兜底（集中在 kindGuidance.ts，扩展新 kind 只改那里）
 */
function deriveQuality(
    ...
    // 降级 2：按 kind 兜底（集中在 kindGuidance.ts）
    return getKindGuidance(node.kind).qualityFallback;
}
...
export async function genPromptsForNode(
    ...
    // 按 node.kind 追加撰写偏好（未注册 kind 走默认空指引，行为不变）
    const kindHint = getKindGuidance(node.kind).promptHint;
    ...
    const nl = [
        ...
        node.intent,
        `数据流转：${plan.summary}`,
        kindHint ? `任务类别撰写偏好（${node.kind}）：${kindHint}` : "",
        "",
        `### 输出契约要求（务必与形态一致）`,
        outputContractHint,
        "",
    ...
    const kindLine = kindHint ? `${kindHint}` : "";
    const outputContractLine = outArr
        ? `输出：请逐条列出所有结果，每条独立成段、以空行分隔，每条自足可理解，不遗漏、不臆造。`
        : `输出：请产出一段连贯完整的文本。`;
    return llmSteps.map((s) => ({
        system: `你是一名严谨的专业数据处理者，负责${s.label}。${goalLine}${kindLine}质量要求：${outQuality.join("；")}。`,
        user: [
            `任务：${node.intent}`,
