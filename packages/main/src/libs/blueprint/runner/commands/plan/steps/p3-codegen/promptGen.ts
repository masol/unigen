...
2. 输入契约：输入数据的形态、含义、来源背景（它是上游哪种加工的产物）。
3. 输出契约：输出的形态、粒度、数量预期、结构/格式要求。下游要拿它做什么，据此定粒度。
4. 边界约定：至少一条异常/空值处理约定（如"若无有效内容，输出空"）。

【形态适配】
- 逐条处理集合中的一条数据：user 说明"你当前处理的是其中一条数据"。
- 一条输入产出多条结果（提取/切分）：user 说明"请以自然语言列出所有结果，每条独立成段，不要遗漏、不要臆造"。
...
export async function genPromptsForNode(
    ...
    const stepsDesc = llmSteps.map((s, i) => {
        ...
        switch (plan.dataFlow) {
            case "map":
                hint = "（处理单条数据，产出一条结果）"; break;
            case "flatmap":
                hint = "（处理单条数据，可能产出多条结果，请全部列出）"; break;
            case "reduce_concat":
                hint = "（逐条处理，各结果将被按序拼接为整体）"; break;
            case "reduce_synthesize":
                hint = i === 0 ? "（逐条处理单条数据）" : "（综合全部结果归纳为单一产物）"; break;
            case "expand":
                hint = "（一次产出多条结果，请分段列出）"; break;
        }
        if (plan.sequential && i === 0 &&
            (plan.dataFlow === "map" || plan.dataFlow === "flatmap" || plan.dataFlow.startsWith("reduce"))) {
            hint += "（存在顺序依赖，会附带前序已处理结果作为上下文）";
        }
    ...
    return llmSteps.map((s) => ({
        ...
        user: [
            ...
            `输出：${node.outputs.join("、")}，请严格符合上述质量要求。`,
