...
const DataId = z.string().regex(/^D-\d{2}$/);
const FuncId = z.string().regex(/^F-\d{2}$/);
const AssumId = z.string().regex(/^A-\d{2}$/);
const Ref = z.string().regex(/^[DF]-\d{2}$/);
export const SrsSchema = z.object({
    ...
    initialData: z
        .array(
            z.object({
                id: DataId,
                name: z.string().describe("数据名称，全文唯一"),
                form: z.string().describe("数据形态，文本优先"),
                required: z.boolean(),
                validation: z.string().describe("可机械执行的校验规则"),
                defaultBehavior: z.string().describe("缺省行为；required 为 true 时填「无」"),
            }),
        )
        .min(1)
        .describe("任务启动时一次性提供的全部输入，不区分数据与配置"),
    functionalRequirements: z
        .array(
            z.object({
                id: FuncId,
                inputs: z.array(Ref).min(1).describe("只能引用 D-xx 或其他 F-xx 的编号"),
                transform: z.string().describe("做什么，一句话，不写怎么做（不提模型/工具/存储）"),
                output: z.string().describe("命名的数据产物，全文唯一"),
