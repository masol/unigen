...
✓ 正确设计：
  ...
  - 如需前后文连贯，intent 注明"顺序执行，参考前一条结果"
...
5. **手动枚举**：是否存在多个节点执行本质相同的操作、仅处理对象不同？

【第 5 点：手动枚举的识别方法】

触发信号（看到任一即怀疑）：
- 节点名含序号/编号（"第一/第二/第三""part1/part2""Q1/Q2"）
- 多个节点 kind 相同且产出结构相同的交付物（如"X草案""Y草案""Z草案"）

确认条件（**必须同时满足**才报告）：
- 这些节点执行的**操作逻辑**本质相同（只是输入数据不同）
- 输入数据可以被抽象为一个列表/集合的不同条目

不报告：操作逻辑不同的同 kind 节点（如"写大纲"和"写初稿"虽然都是 generate 但操作逻辑不同）。
...
手动枚举的报告格式：
\`ISSUE: [相关节点名列表] 手动枚举同类操作——应将处理对象抽象为 isArray=true 的交付物，合并为单个节点消费 array 输入、产出 array 输出。如需顺序执行在 intent 中注明"顺序执行，参考前一条结果"。\`
...
/**
 * 去除名称中的序号/编号，用于手动枚举检测
 */
function stripOrdinal(name: string): string {
    return name
        .replace(/第[一二三四五六七八九十百千\d]+[幕章节部分篇条项段落]/g, '')
        .replace(/part\s*\d+/gi, '')
        .replace(/[Qq]\d+/g, '')
...
export function validateNodes(
    ...
    for (const n of nodes) {
        ...
        // 规则1：非 extract/split 节点不能凭空产出数组
        ...
        // 规则2：map 语义节点（输入数组→输出数组）输入输出名不能相同
        const hasArrayInput = n.inputs.some(i => artMap.get(i.name)?.isArray === true);
        if (hasArrayInput && outArt.isArray && n.outputs[0].name === n.inputs[0].name) {
            errors.push(
    ...
    // ── 手动枚举检测（程序化兜底） ──────────────────────────────────────
    const kindGroups = new Map<string, DagNode[]>();
    for (const n of nodes) {
        const group = kindGroups.get(n.kind) ?? [];
        group.push(n);
        kindGroups.set(n.kind, group);
    }
    ...
    for (const [kind, group] of kindGroups) {
        ...
        // 按去序号后的输出名分组
        const outputGroups = new Map<string, DagNode[]>();
        for (const n of group) {
            if (n.outputs.length !== 1) continue;
            const outArt = artMap.get(n.outputs[0].name);
            if (outArt?.isArray) continue; // 已经是数组标注的不管
            const stripped = stripOrdinal(n.outputs[0].name);
            if (!stripped) continue; // 去序号后为空说明不是序号命名
            const sub = outputGroups.get(stripped) ?? [];
            sub.push(n);
            outputGroups.set(stripped, sub);
        }

        for (const [stripped, sub] of outputGroups) {
            if (sub.length < 3) continue;
            const names = sub.map(n => `「${n.name}」`).join("");
            errors.push(
                `${names} 疑似手动枚举同类操作（kind=${kind}，输出去序号后均为"${stripped}"），` +
                `应合并为单节点 + isArray 输入/输出`);
        }
    }
...
// ─── 登记（含 sequential 自动推断） ────────────────────────────────────────

/**
 * 检测 intent 中的顺序执行关键词
 */
function detectSequential(intent: string): boolean {
    const keywords = ['顺序执行', '参考前一条', '前一条结果', '依赖上一条', '串行', '逐条', '依次'];
...
export async function registerLayer(
    ...
    for (const n of result.nodes) {
        ...
        // 自动推断 sequential：输入含 array 且 intent 含顺序关键词
        const hasArrayInput = n.inputs.some(i => i.isArray);
        const sequential = hasArrayInput && detectSequential(n.intent);
        ...
        pnodes.push({
            ...
            error: null,
            facets: {},
            sequential: sequential || undefined, // undefined 节省存储
        });
    }
