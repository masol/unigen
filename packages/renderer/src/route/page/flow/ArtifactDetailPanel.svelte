<!-- src/lib/flow/ArtifactDetailPanel.svelte -->
<script lang="ts">
    ...
    >
      ...
      <div class="flex items-start justify-between gap-2 p-4 pb-0">
        ...
        <Button
          ...
          onclick={() => flowStore.selectArtifact(null)}
        >
          <IconX size={16} stroke={1.5} />
        </Button>
      </div>

      <!-- 原生滚动容器，规避 Panel+motion 时序崩溃 -->
      <div class="max-h-96 space-y-4 overflow-y-auto p-4">
        {#if art.artifact}
          ...
          <div class="text-[11px] text-muted-foreground">
            Schema：{art.artifact.dataSchema === null
              ? "尚未细化（第一遍恒为 null）"
              : "已细化"}
          </div>
        {:else}
